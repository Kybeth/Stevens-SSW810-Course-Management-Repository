...
# Below are the HW11 new features
def open_db():
    """ Connect to the database """
    DB_FILE = '/Users/kybeth/Documents/SIT/SSW_810/HW11/810_startup.db'
    db = sqlite3.connect(DB_FILE)
    return db

def db_instructor_summary():
    db = open_db()
    print('Instructor Summary from Database')
    pt = PrettyTable(field_names=['CWID', 'Name', 'Dept', 'Course', 'Students'])
    for row in db.execute('select ins.CWID, ins.Name, ins.Dept, gra.Course, count(gra.Student_CWID) from HW11_instructors ins join HW11_grades gra on ins.CWID=gra.Instructor_CWID group by gra.course order by ins.CWID'):
        pt.add_row(row)
    print(pt) 
